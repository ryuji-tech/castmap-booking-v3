...
不調だったインターネットがついに復旧した。（←個人の話なので読み飛ばしてOK）今回は純粋にCRUD処理としてログインの実装を行った。Laravelを少しかいつまんで学習してみたがPHPでのCRUD処理の理解があってこそだと思った。よって今回は前回のv2から大幅な改修は行わず基本的なアップデートに専念した。
